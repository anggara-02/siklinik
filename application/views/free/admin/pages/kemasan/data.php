	...
	<div class="modal-dialog modal-dialog-centered">
		...
		<form class="form-horizontal" id="FormData" method="POST" action="<?= site_url('admin/'.$class.'/action_stored');?>" enctype="multipart/form-data">
			...
			<div class="modal-body"> 
				<div class="form-group kemasan_name-field">
					<label for="" class="form-label">Kemasan</label>
					<input type="text" name="kemasan_name" id="kemasan_name" class="form-control" value="">
				</div>
			</div>
			<div class="modal-footer">
				<input type="hidden" name="kemasan_id" id="kemasan_id" class="form-control" value="0">
...
<script>
	$(document).ready(function() { 
		...
		var validator = $('#FormData').validate({
			rules: {
				kemasan_name: {
					required: function(element) {
						return $('.kemasan_name-field').addClass('is-invalid');
					},
					remote: {
						url: "<?= site_url('admin/'.$class.'/action_validation');?>",
						type: "post",
						data: {
							kemasan_name: function() {
							return $("#kemasan_name").val();
							},
							kemasan_id: function() {
							return $("#kemasan_id").val();
							}
						}
					}
				} 
            ...
            messages:{
                kemasan_name: {
                    remote: "Kemasan sudah di gunakan"
                }
            }
		});
	});
		...
		$.ajax({
			...
			success:function(response){
				if(response['status']==200) {
					$('#FormData').find('[name="kemasan_name"]').val(response['data']['kemasan_name']);
					$('#FormData').find('[name="kemasan_id"]').val(response['data']['kemasan_id']);
	...
	function load_table() {
		...
		var link_service='?kemasan_name='+keyword;
	...
	function dataReload(link_service){
		...
		'columns':[
			...
			{data: "kemasan_name", visible: true}
		],
		});
