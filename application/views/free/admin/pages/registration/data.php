...
<div class="section-body">
 ...
 <script>
            ...
            function validasi_delete(id) {
				...
				.then((confirm) => {
					if (confirm) {
						window.location.href ="<?php echo site_url('admin/registration/delete');?>/"+id;
						return true;
					}
					else
					{
			...
			function send_to_pemeriksaan(status,pendaftaran_id) {
				text_status=(status=='pemeriksaan')?'Pemeriksaan':'-';
				swal({
					title: 'Apakah anda yakin?',
				...
				.then((confirm) => {
					if (confirm) {
						$.ajax({
						   url:'<?php echo site_url('admin/'.$class.'/send_to_pemeriksaan');?>',
						   type:'POST',
						   dataType:'json',
			...
			function dataReload(link_service){
			  $('#datatables').DataTable({
				...
				'columns':[
					...
					{ title: "No Antrian", data: "pendaftaran_no", visible: true},
					...
					{ title: "Status", data: "pendaftaran_status", visible: true}
				],
			  });
