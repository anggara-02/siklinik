					...
					<table id="datatables" class="table table-sm table-bordered text-nowrap" style="width: 100%">
						<thead>
							<tr class="bg-light">
								...
								<th class="align-middle text-center">Jenis Penjamin</th>
							</tr>
						</thead>
					</table>
				</div> 
	...
	<div class="modal-dialog modal-dialog-centered">
		...
		<form class="form-horizontal" id="FormData" method="POST" action="<?= site_url('admin/'.$class.'/action_stored');?>" enctype="multipart/form-data">
			...
			<div class="modal-body">
				<div class="form-group penjamin_name-field">
					<label for="" class="form-label">Jenis Penjamin</label>
					<input type="text" name="penjamin_name" id="penjamin_name" class="form-control" value="">
				</div>
			</div>
			<div class="modal-footer">
				<input type="hidden" name="penjamin_id" id="penjamin_id" class="form-control" value="0">
...
<script>
	$(document).ready(function() { 
		...
		var validator = $('#FormData').validate({
			rules: {
				penjamin_name: {
					required: function(element) {
						return $('.penjamin_name-field').addClass('is-invalid');
					},
                    remote: {
						url: "<?= site_url('admin/'.$class.'/action_validation');?>",
						type: "post",
						data: {
							penjamin_name: function() {
							return $("#penjamin_name").val();
							},
							penjamin_id: function() {
							return $("#penjamin_id").val();
							}
						}
					} 
				}
            ...
            messages:{
                penjamin_name: {
                    remote: "Jenis Penjamin sudah di gunakan"
                }
            }
		});
	});
	...
	function edit_form(penjamin_id) {
		$.ajax({
			...
			success:function(response){
				if(response['status']==200) {
					$('#FormData').find('[name="penjamin_name"]').val(response['data']['penjamin_name']);
					$('#FormData').find('[name="penjamin_id"]').val(response['data']['penjamin_id']);
	...
	function load_table() {
		...
		var link_service='?penjamin_name='+keyword;
	...
	function dataReload(link_service){
		...
		'columns':[
			...
			{data: "penjamin_name", visible: true}
		],
		});
