		  ...
		  <?php 
				...
				<table class="table">
					<tbody> 
						<tr>
							...
							<td  width="40%">
								<input type="text" id="pasien_name" autocomplete="off" name="pasien_name" class="form-control input-sm" value="<?php echo $this->input->post('pasien_name')?$this->input->post('pasien_name'):(isset($pendaftaran_pasien_name)?$pendaftaran_pasien_name:'-');?>">
							</td>
							<td width="10%">Usia</td>
							<td width="40%"><input type="number" id="pasien_birthdate" autocomplete="off" name="pasien_birthdate" class="form-control input-sm" value="<?php echo $this->input->post('pasien_birthdate')?$this->input->post('pasien_birthdate'):(isset($pendaftaran_pasien_pernikahan)?$pendaftaran_pasien_pernikahan:'-');?>"></td>
						</tr> 
						<tr>
							<td width="10%">Telp</td>
							<td width="40%">
								<input type="text" id="pendaftaran_pasien_penanggung_jawab_telp" autocomplete="off" name="pendaftaran_pasien_penanggung_jawab_telp" class="form-control input-sm" value="<?php echo $this->input->post('pendaftaran_pasien_penanggung_jawab_telp')?$this->input->post('pendaftaran_pasien_penanggung_jawab_telp'):(isset($pendaftaran_pasien_penanggung_jawab_telp)?$pendaftaran_pasien_penanggung_jawab_telp:'-');?>">
							</td>
							<td width="10%">Alamat</td>
							<td width="40%">
							<textarea type="text" id="pasien_address" autocomplete="off" name="pasien_address" class="form-control input-sm"><?php echo $this->input->post('pasien_address')?$this->input->post('pasien_address'):(isset($pendaftaran_pasien_address)?$pendaftaran_pasien_address:'-');?></textarea>
							</td>
						</tr> 
						<tr>
							<td width="10%">Membership</td>
							<td width="40%">
								<select type="text" id="pasien_membership_id" autocomplete="off" name="pasien_membership_id" class="form-control input-sm">
									<option value='0'>Umum</option>
									<?php foreach($membership_arr as $row){?>
										<option value='<?php echo $row['id'];?>' <?php echo $this->input->post('pasien_membership_id')==$row['id']?'selected':(isset($pendaftaran_membership_id)&&$pendaftaran_membership_id==$row['id']?'selected':'');?>><?php echo ucwords($row['membership_name']);?></option>
									<?php } ?>
								</select>
							</td>
			...
			<div class="form-group row mb-5">
				...
				<div class="col-sm-12 col-md-8">
					<input type="hidden" class="form-control" name="pendaftaran_id" value="<?php echo (isset($pendaftaran_id)?$pendaftaran_id:'');?>"> 
			...
			<div class="table-responsive">
				<table class="table">
					...
					foreach($pemeriksaan_arr as $arr){
						if($temp!=$arr['pendaftaran_date'])
						{
							$no++;
						}
						?>
						<tr>
							<td><?php echo ($temp!=$arr['pendaftaran_date'])?$no:'';?></td>
							<td>
								<?php echo ($temp!=$arr['pendaftaran_date'])?date('d-m-Y H:i:s',strtotime($arr['pendaftaran_date'])):'';?>
							</td>
							<td>
								<?php echo trim($arr['pemeriksaan_obat_name']);?>
							</td>
							<td>
								<?php echo trim($arr['pemeriksaan_obat_qty']);?>
							</td>
							<td>
								<?php echo trim($arr['pemeriksaan_obat_kemasan_name']);?>
							</td>
							<td>
								<?php echo common_lib::currency_format($arr['pemeriksaan_obat_price']);?>
							</td>
							<td>
								<?php echo common_lib::currency_format($arr['pemeriksaan_obat_price']*$arr['pemeriksaan_obat_qty']);?>
							</td>
						</tr>
					<?php
						$temp=$arr['pendaftaran_date'];
					} ?>
					</tbody>
				</table>
			</div>
