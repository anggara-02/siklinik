...
class poli extends controller_admin {
    ...
    public function get_datatable() {
        ...
        foreach ($query->result() as $row) {
			...
			$no++;
			$action='<a href="#" onclick="edit_form('.$poli_id.')" style="margin-right:5px" class="btn btn-info btn-icon"><i class="fa fa-edit"></i></a>';
			$action.='<a href="javascript:void(0);" data-url="'.site_url('admin/'.__class__.'/delete/'.$poli_id.'').'" class="btn btn-danger btn-icon btn-delete confirmation"><i class="fa fa-trash"></i></a>';
			
			$entry = array('no'=>$no, 'action'=>$action, 'poli_name'=>$poli_name,);
    ...
    public function delete($id=0) {   
		common_lib::hak_akses($this->access,__function__,'controller'); 
		$has_exist=$this->common_lib->select_row('data_poli','poli_id='.intval($id));
		if(empty($has_exist)) {
			$this->session->set_flashdata('status', '500');
			$this->session->set_flashdata('gagal', 'Data gagal di hapus');
			redirect('admin/poli/data');
		}
		$result=$this->common_lib->delete_semu('data_poli',['poli_id'=>intval($id)]);
	...
	public function action_validation() {
		if ($_POST['poli_id']) {
			$query = $this->db->query("SELECT data_poli.* FROM data_poli WHERE is_delete=0 AND poli_name = '{$_POST['poli_name']}' AND poli_id = '{$_POST['poli_id']}' LIMIT 1")->result();
			if ($query) {
				echo json_encode(true);
			} else {
				$query = $this->db->query("SELECT poli_name FROM data_poli WHERE is_delete=0 AND poli_name = '{$_POST['poli_name']}' AND poli_id != '{$_POST['poli_id']}' LIMIT 1")->result();
				echo $query ? json_encode(false) : json_encode(true);
			}
		} else {
			$query = $this->db->query("SELECT poli_name FROM data_poli WHERE is_delete=0 AND poli_name = '{$_POST['poli_name']}' LIMIT 1")->result();
			echo $query ? json_encode(false) : json_encode(true);
		}
	}
	
	public function action_stored() { 
		$id=isset($_POST['poli_id'])?$_POST['poli_id']:0;
		if(intval($id)>0) {
