...
class Stencil {
	...
	public function paint($template,$page, $data = NULL)
	{
		...
		foreach ($this->slice as $key => $value)
		{
			if (is_array($value))
			{
				foreach ($value as $k => $v)
				{
					...
					$this->data[$k] = $this->CI->load->view($template.'/slices/'.$v, $this->data, TRUE)."\n";
				}
			}
			elseif (!is_numeric($key))
			{
				...
				$this->data[$key] = $this->CI->load->view($template.'/slices/'.$value, $this->data, TRUE)."\n";
			}
			else
			{
				...
				$this->data[$value] = $this->CI->load->view($template.'/slices/'.$value, $this->data, TRUE)."\n";
			}
		}
		$this->data['content'] = $this->CI->load->view($template.'/pages/'.$page, $this->data, TRUE)."\n";
		$this->CI->load->view($template.'/'.$this->layout, $this->data);
	}
